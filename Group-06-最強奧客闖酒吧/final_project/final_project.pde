...
void resetAll(){
  ...
  timerCard = new Timer(1800); //30sec 
  playerDiceNum = 1;
...
void draw() {
  ...
  case GameState.STAGE11: // stage1 play
    ...
    switch(stageState) {
      ...
      case DiceStage.ANIMATE: // dice animate display
    ...
    case DiceStage.PLAY: // dice bar move and detect win lose
      ...
      if (dice.diceWin==1) {
        textFont(myFont);
        fill(#FFFFFF);
        text("YOU WIN! NEXT STAGE", 140, 180);
        count++;
        if (count >= 240) {
  ...
  case GameState.STAGE31: //剪刀石頭布
    ...
    case PSSstage.PLAY: //猜拳
      ...
      if(pss.pssWinNum == 1){
        ...
        textFont(myFont);
        textSize(28);
        text("YOU WIN! SLAP THE BOSS", 150, 180);
        if( count >= 180){
      ...
      if(pss.pssWinNum == 3){
        count ++;
        textSize(28);
        text("YOU LOSE! DEFEND YOURSELF", 120, 180);
        if( count >= 180){
